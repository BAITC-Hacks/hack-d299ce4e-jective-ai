...
Интерфейс: http://localhost:3000. По умолчанию используются демоданные, backend запускать не нужно.
Остановка — `Ctrl+C` в терминале.

Для одновременного запуска frontend и API:

```bash
npm run dev:full
```

Эта команда автоматически включает API-режим каталога. Frontend остаётся на порту 3000,
API работает на http://127.0.0.1:3001. `Ctrl+C` останавливает оба процесса.
...
| Команда                     | Назначение                                                           |
| --------------------------- | -------------------------------------------------------------------- |
| `npm run dev` / `npm start` | Frontend в деморежиме, если `.env.local` не переопределяет настройки |
...
Запустите `npm run dev:api` и `npm run dev` в двух терминалах. Vite перенаправляет `/api`
...
Все три маршрута поддерживают `HEAD`. Ошибки имеют вид
...
Настоящая авторизация, запись задач, отклики, AI и база данных ещё не реализованы.
Эти действия работают в текущей демосессии и сбрасываются после перезагрузки страницы.
API-режим подключает серверное чтение каталога; он не превращает остальные демодействия в серверные.
